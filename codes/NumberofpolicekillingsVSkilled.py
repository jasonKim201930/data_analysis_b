#Number of police killings VS Number of police killed
...
#murder_total_df=murder_state_total.to_frame()
#murder_state_total=pd.merge(df,df_winning,on='State')

#murder_state_index=police_killing_total.groupby('State')["Victim's name"].nunique().in

#murder_state_total.plot.bar(color=murder_state_total.winner.map({1:'r',0:'b'}))

#plt.bar(murder_state_index,murder_state_total,color=murder_total["winner"].map(
#print(murder_state_total)
#print(murder_total_df)
#murder_state_total.plot.bar(color=murder_total_df.winner.map({1:'r', 0:'b'}))
